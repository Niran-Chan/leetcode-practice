...
public:
       void dfs (vector<vector<int>>& heights, int row, int col, int 
height, vector<vector<int>>& visited) {
        if (row < 0 || row >= heights.size() || col < 0 || col >= 
heights[row].size()) return;
    ...
    vector<vector<int>> pacificAtlantic(vector<vector<int>>& heights) {
        ...
        // will start from the boundaries:
        // (1) check which nodes can be reach to Atlantic, and save into 
one vector
        // (2) check which nodes can be reach to Pacific, and save into 
another vector
        // (3) nodes are in both vectors are the result
        ...
        for (int i = 0; i < m; ++i) {
            for (int j = 0; j < n; ++j) {
                if (pacific[i][j] && atlantic[i][j]) result.push_back({i, 
j});
            }
        }
        return result;
