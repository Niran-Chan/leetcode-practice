...
public:
    ...
    // DFS function to traverse the graph and calculate the minimum fuel 
cost
    ll dfs(vector<int>& vis, int node, int seats, vector<int> adj[]){
        ...
        // calculate the fuel cost required to visit all nodes reached so 
far
        ll x = count / seats;
        if(count % seats) x++;
        if(node != 0) ans += x;
    ...
    long long minimumFuelCost(vector<vector<int>>& roads, int seats) {
        int n = roads.size();
        vector<int> adj[n+1], vis(n+1, 0); // adjacency list and visited 
array
